//-----------------------------------------------------------------------
// <copyright file="MsnClient.cs" company="The Outercurve Foundation">
...
// <website>https://github.com/msn-csharp-sdk/msn-csharp-sdk</website>
//-----------------------------------------------------------------------
...
namespace Msn
{
    ...
    public partial class MsnClient
    {
        ...
        private const string AttachmentMustHavePropertiesSetError = "Attachment (MsnMediaObject/MsnMediaStream) must have a content type, file name, and value set.";
        private const string AttachmentValueIsNull = "The value of attachment (MsnMediaObject/MsnMediaStream) is null.";
        private const string UnknownResponse = "Unknown msn response.";
        ...
        /// <summary>
        /// Gets or sets the value indicating whether to use Msn beta.
        /// </summary>
        public virtual bool UseMsnBeta
        {
            get { return _useMsnBeta; }
            set { _useMsnBeta = value; }
        }
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="MsnClient"/> class.
        /// </summary>
        static MsnClient()
        {
        ...
        /// Initializes a new instance of the <see cref="MsnClient"/> class.
        /// </summary>
        /// <param name="accessToken">The msn access_token.</param>
        /// <exception cref="ArgumentNullException">Access token in null or empty.</exception>
        public MsnClient(string accessToken)
            : this()
        {
        ...
        private HttpHelper PrepareRequest(HttpMethod httpMethod, string path, object parameters, Type resultType, out Stream input, out bool containsEtag, out IList<int> batchEtags)
        {
            ...
            uriBuilder.Host = path.Contains("oauth20_token.srf") ? "login.live.com" : "apis.live.net";
            ...
            if (httpMethod != HttpMethod.Post)
            {
                ...
                if (mediaObjects.Count > 0 && mediaStreams.Count > 0)
                    throw new InvalidOperationException("Attachments (MsnMediaObject/MsnMediaStream) are valid only in POST requests.");
            ...
            else
            {
                ...
                else
                {
                    ...
                    foreach (var msnMediaObject in mediaObjects)
                    {
                        var sbMediaObject = new StringBuilder();
                        var mediaObject = msnMediaObject.Value;
                        ...
                        sbMediaObject.Append("Content-Disposition: form-data; name=\"").Append(msnMediaObject.Key).Append("\"; filename=\"").Append(mediaObject.FileName).Append("\"").Append(MultiPartNewLine);
                    ...
                    foreach (var msnMediaStream in mediaStreams)
                    {
                        var sbMediaStream = new StringBuilder();
                        var mediaStream = msnMediaStream.Value;
                        ...
                        sbMediaStream.Append("Content-Disposition: form-data; name=\"").Append(msnMediaStream.Key).Append("\"; filename=\"").Append(mediaStream.FileName).Append("\"").Append(MultiPartNewLine);
            ...
            request.TrySetUserAgent("Msn C# SDK");
        ...
        internal static Exception GetException(HttpHelper httpHelper, object result)
        {
            ...
            MsnApiException resultException = null;
            ...
            if (httpHelper != null)
            {
                ...
                // legacy rest api
                if (responseUri.Host == "api.msn.com" ||
                    responseUri.Host == "api-read.msn.com" ||
                    responseUri.Host == "api-video.msn.com" ||
                    responseUri.Host == "api.beta.msn.com" ||
                    responseUri.Host == "api-read.beta.msn.com" ||
                    responseUri.Host == "api-video.msn.com")
                {
                    if (responseDict.ContainsKey("error_code"))
                    {
                        ...
                        // Error Details: http://wiki.developers.msn.com/index.php/Error_codes
                        if (errorCode == "190")
                            resultException = new MsnOAuthException(errorMsg, errorCode);
                        else if (errorCode == "4" || errorCode == "API_EC_TOO_MANY_CALLS" ||
                                 (errorMsg != null && errorMsg.Contains("request limit reached")))
                            resultException = new MsnApiLimitException(errorMsg, errorCode);
                        else
                            resultException = new MsnApiException(errorMsg, errorCode);
            ...
            if (responseDict.ContainsKey("error"))
            {
                ...
                if (error != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(errorType) && !string.IsNullOrEmpty(errorMessage))
                    {
                        ...
                        // It is easier to understand the error if we use Msn's error message.
                        if (errorType == "OAuthException")
                            resultException = new MsnOAuthException(errorMessage, errorType, errorCode);
                        else if (errorType == "API_EC_TOO_MANY_CALLS" || (errorMessage.Contains("request limit reached")))
                            resultException = new MsnApiLimitException(errorMessage, errorType);
                        else
                            resultException = new MsnApiException(errorMessage, errorType, errorCode);
                    }
                }
                else
                {
                    ...
                    {
                        if (errorNumber == 190)
                            resultException = new MsnOAuthException(errorDescription, "API_EC_PARAM_ACCESS_TOKEN");
                        else
                            resultException = new MsnApiException(errorDescription, errorNumber.Value.ToString(CultureInfo.InvariantCulture));
                    }
                }
            }
        ...
        /// <param name="mediaObjects">The extracted Msn media objects.</param>
        /// <param name="mediaStreams">The extracted Msn media streams.</param>
        /// <returns>The converted dictionary.</returns>
        private static IDictionary<string, object> ToDictionary(object parameters, out IDictionary<string, MsnMediaObject> mediaObjects, out IDictionary<string, MsnMediaStream> mediaStreams)
        {
            mediaObjects = new Dictionary<string, MsnMediaObject>();
            mediaStreams = new Dictionary<string, MsnMediaStream>();
            ...
            {
                if (parameter.Value is MsnMediaObject)
                    mediaObjects.Add(parameter.Key, (MsnMediaObject)parameter.Value);
                else if (parameter.Value is MsnMediaStream)
                    mediaStreams.Add(parameter.Key, (MsnMediaStream)parameter.Value);
            }
        ...
        private static string BuildHttpQuery(object parameter, Func<string, string> encode)
        {
            ...
            else
            {
                ...
                if (mediaObjects.Count > 0 || mediaStreams.Count > 0)
                    throw new InvalidOperationException("Parameter can contain attachements (MsnMediaObject/MsnMediaStream) only in the top most level.");
