//-----------------------------------------------------------------------
// <copyright file="MsnClient.Async.Tasks.cs" company="The Outercurve Foundation">
...
#if ASYNC_AWAIT
, IProgress<MsnUploadProgressChangedEventArgs> uploadProgress
#endif
)
        {
...
#if ASYNC_AWAIT
                EventHandler<MsnUploadProgressChangedEventArgs> uploadProgressHandler = null;
                    ...
                    uploadProgressHandler = (sender, e) =>
                                            {
                                                if (e.UserState != tcs)
                                                    return;
                                                uploadProgress.Report(new MsnUploadProgressChangedEventArgs(e.BytesReceived, e.TotalBytesToReceive, e.BytesSent, e.TotalBytesToSend, e.ProgressPercentage, userState));
                                            };
            ...
            EventHandler<MsnApiEventArgs> handler = null;
            handler = (sender, e) =>
            {
        ...
        private void RemoveTaskAsyncHandlers(HttpMethod httpMethod, EventHandler<MsnApiEventArgs> handler)
        {
            switch (httpMethod)
            {
                case HttpMethod.Get:
...
#endif
        /// <returns>The json result task.</returns>
        public virtual Task<object> PostTaskAsync(string path, object parameters, object userState, CancellationToken cancellationToken, IProgress<MsnUploadProgressChangedEventArgs> uploadProgress)
        {
